...
define('jobs/hostGroup',function(require,exports,module){
	...
	var hostGroupData, flagname, flagSave, srcTarget,hgname = 0;
    ...
    var exports = {
        ...
        btntpl :'{{#groupname}}<button type="button" class="btn btn-sm btn-success" group="{{name}}">{{name}}</button>  {{/groupname}}',  
        init:function(){
            this.getHostGroup();
            this.bindEvent();
        },
        bindEvent:function() {
            ...
            exports.nodes.ahgp.on('keyup',this.saveHostGroupName);
        ...
        saveHostGroupName:function(event){
            if(event.keyCode == 13){
                ...
                exports.nodes.hw.find('button[group="'+ saveName +'"]').text(sendVal);

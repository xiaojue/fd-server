		...
		initToggle:function(hosts){
			...
			for(var i in hosts){
				if(hosts.hasOwnProperty(i)){
					...
					var txt = (status == 1) ? 'hide' : 'show';
					var fun = (status == 1) ? 'show' : 'hide';
                ...
                $.ajax({
                    ...
                    success: function (result){
                        if(result && result.key){
                            if(!self.socket){
                                self.socket = io.connect('http://fd.server:54321');
                    ...
                    success: function(result) {
                        if(result){
                            if(result.data){
                                ...
                                $("#task .path_error").html(result.error||'找不到有效任务').show();
                            }
                        }
                    }
                });
            }else{
                $("#task .path_error").html("路径错误~！").show();
            }
        },
        updateList: function (data){
            var el = $("#task .tasklist");
            var htmlStr = '<p data-node="checkboxs">';
            for(var file in data){
                htmlStr += file + ': ';
                var tasks = data[file];
                for(var i = 0; i < tasks.length; i++){
                    var taskName = tasks[i];
                    var val = file + ':' + taskName;
                    htmlStr += '<input type="checkbox" value="'+ val +'">' + taskName;
                }
                htmlStr += '<br/>';
            }
            htmlStr += '</p>';
            htmlStr += '<button data-action="runtask" type="button" class="btn btn-sm btn-success mr">Run</button>';
            el.html(htmlStr).show();
            $("#task .path_error").html('加载成功！');
        },
		el:'#task',

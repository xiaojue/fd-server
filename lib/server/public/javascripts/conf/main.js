	...
	var proxy = backbone.View.extend({
		...
		editGroup: function(e) {
			...
			if (groupname) {
				model.fetch({
					success: function() {
						var groups = model.get('proxyGroup');
						groups = _.without(groups, groupname);
						model.save('proxyGroup', _.union(groups, [newname]), {
							success: function() {
								model.trigger('change:proxy');
							}
						});
	...
	var hosts = backbone.View.extend({
		...
		editHost:function(e){
			...
			dialog.init({
				...
				toText: 'ip',
				success: function(srcval, toval, errnode) {
					var data = {'domain':srcval,'ip':toval,'disabled':disabled,'groupname':groupname};
					$.ajax({
						url:'/hostFile',
						type:'post',
						dataType:'json',

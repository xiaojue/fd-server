	...
	readDirFile : function (req, files, dirs) {
		...
		if(files && files.length != 0){
			for(var i = 0; i< files.length; i++){
				var path = Path.join(req.url,files[i]);
				responseStruc += '<a href="http://'+ req.headers.host + path +'">' + files[i] + '</a><br/>';
			}
		}
		if(dirs){
			for(var i in dirs){
				var path = Path.join(req.url,i);
				responseStruc += '<a href ="http://'+ req.headers.host + path +'">' + i + '/' +'</a><br/>';
			}
		}
		return responseStruc || '查找文件失败';
	},
			...
			fileServer.serve(req, res, function(err, result) {
				if (err && (err.status === 404)) {
					if (openOnlineProxy === 0) {
						var structure, files, dirs, flag;

						if(!self.vhostTree[path]){
							//查找根目录下的文件和列表
							structure = dstruc.sync(path, {
								recursive: true,
								extensionAsKey: false
							});
							self.vhostTree[path] = structure;
							files = structure.files;
							dirs = structure.dirs;
						}else{
							files = self.vhostTree[path].files;
							dirs = self.vhostTree[path].dirs;
						}
						//此处浏览器默认查找当前路径下是否有index.html，如果有的话直接打开改页面
						if(url.parse(req.url).pathname === '/'){
							var html = self.readDirFile(req, files, dirs);
							res.writeHeader(200,{'content-type':'text/html'});
							res.end(html);
						}else if(url.parse(req.url).pathname != '/'){
							//匹配二级或三级目录
							var dirItem = url.parse(req.url).pathname.split('/');
							var mulu;
							for(var i = 0; i< dirItem.length; i++){
								if(dirItem[i]){
									if(!mulu){
										...
										mulu = mulu.dirs[dirItem[i]];
									}
								}
							}
							var html = self.readDirFile(req, mulu && mulu.files, mulu && mulu.dirs);
							res.end(html);
						}else{

...
fdMaster.prototype = {
	...
	readDirFile : function (req, files, dirs) {
		...
		if(dirs){
			for(var j in dirs){
				path = Path.join(req.url,j);
				responseStruc += '<a href ="http://'+ req.headers.host + path +'">' + j + '/' +'</a><br/>';
			}
		}
				...
				if (err && (err.status === 404)) {
					if (openOnlineProxy === 0) {
						...
						if(url.parse(req.url).pathname === '/'){
							...
							html = self.readDirFile(req, mulu && mulu.files, mulu && mulu.dirs);
							res.end(html);
						}else{
	...
	setupVhost: function(cb) {
		...
		async.each(this.vhosts, function(item, callback) {
			...
			if (path && fs.existsSync(path)) {
				...
				var fileServer = new nodeStatic(path,{gzip:true});

...
fdMaster.prototype = {
    ...
    _batchData: function(data, callback) {
        //初始化vhost配置数据
        var self = this;
        this.vhosts = [];
        this.proxy = [];
        ...
        async.eachSeries(Object.keys(data.vhost), function(domain, cb) {
            var v = data.vhost[domain];
            if (v.port) {
                self.vhosts.push({

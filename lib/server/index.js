...
utils.extend(fdMaster.prototype, {
  ...
  setup: function() {
    ...
    this.configManager.on('change', function(json,diff) {
      ...
      diff.forEach(function(item){
        ...
        if(self.pluginNames.indexOf(changeName)) restartServices.push(changeName); 
      });
  ...
  */
  ...
  start: function(services,cb) {
    var self = this,plugins = self.plugins.filter(function(item){
      return services.indexOf(item.name); 
    });
    //要先开bouncy
